        ...
        if args.dataset in ['cifar100', 'mini_imagenet']:
            for cls in classes[classes_int]:
                if args.pret_clip:
                    ...
                    output = Bert_model.text_encoder.encode_text(
                        encoded_input.to(next(Bert_model.parameters()).device)
                    )
                    ...
                    # 这里把数据转移到GPU中，否则由于Bert_model在GPU上会报错
                    device = next(Bert_model.parameters()).device
                    encoded_input = tokenizer(f'a photo of {cls}', return_tensors='pt').to(device)
...
def base_train(model, trainloader, optimizer, scheduler, epoch, class_list, args):
    ...
    for i, batch in enumerate(tqdm_gen, 1):
        ...
        # # 拉近key和x之间的距离
        # if args.prompt_pool:
        #     # total_loss = total_loss - args.pull_constraint_coeff * res['reduce_sim']

        #     # 计算动态缩放因子
        #     reduce_sim_scale = total_loss.abs().detach().mean() / (res['reduce_sim'].abs().detach().mean() + 1e-8)
        #     # 缩放后的 reduce_sim
        #     scaled_reduce_sim = res['reduce_sim'] * reduce_sim_scale
        #     # 最终损失
        #     total_loss = (1 - args.pull_constraint_coeff) * total_loss - args.pull_constraint_coeff * scaled_reduce_sim
        ...
        # for p in model.meta_net_3.parameters():
        #     print(p.grad)
...
def test(model, testloader, epoch, args, session, base_acc=0.0, Lase_No_Loss=False, Mytest=False):
