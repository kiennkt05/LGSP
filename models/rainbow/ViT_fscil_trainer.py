...
from models.rainbow.ViT_Network import ViT_Rainbow, ViT_Hybrid
...
class ViT_HybridTrainer(Trainer):
    def __init__(self, args):
        super().__init__(args)
        self.args = args
        self.device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
        ...
        self.model = ViT_Hybrid(self.args, mode=self.args.base_mode)
        self.model = self.model.to(self.device)
    ...
    def get_optimizer_base(self):
        ...
        if getattr(self.args, 'pixel_prompt', 'NO') == "YES":
            ...
            params_mask = [p for p in prompt_branch_params if p.requires_grad]
            if params_mask:
                optimizer_params.append({'params': params_mask, 'lr': getattr(self.args, 'lr_local', 0.01)})

        # Frequency mask parameters
        if getattr(self.args, 'Frequency_mask', False) and hasattr(self.model, 'weights'):
            optimizer_params.append(
                {'params': [self.model.weights], 'lr': getattr(self.args, 'lr_Frequency_mask', 0.03)}
            )

        # Adaptive weighting parameters
        if getattr(self.args, 'adaptive_weighting', False) and hasattr(self.model, 'alpha') and hasattr(self.model, 'beta'):
            optimizer_params.append({'params': [self.model.alpha, self.model.beta], 'lr': 0.1})

        # Rainbow prompt parameters (only layers 3–5)
        num_layers = len(self.model.encoder.blocks)
        rainbow_start_layer = 3
        rainbow_end_layer = 5
        rainbow_start_idx = max(0, rainbow_start_layer - 1)
        rainbow_end_idx = min(num_layers - 1, rainbow_end_layer - 1)

        for layer_idx in range(rainbow_start_idx, rainbow_end_idx + 1):
            for prompt in self.model.rainbow_prompt.base_prompts[layer_idx]:
                if prompt.requires_grad:
                    optimizer_params.append({'params': [prompt], 'lr': self.args.lr_base})
        ...
        # VPT prompt tokens (higher layers)
        if hasattr(self.model, 'Prompt_Tokens') and self.model.Prompt_Tokens is not None:
            optimizer_params.append(
                {'params': [self.model.Prompt_Tokens], 'lr': getattr(self.args, 'lr_PromptTokens_base', 2e-4)}
            )
        ...
        for session in range(args.start_session, args.sessions):
            ...
            if session == 0:
                ...
                for epoch in range(args.epochs_base):
                    ...
                    start_time = time.time()
                    train_loss, train_acc = base_train(
                        self.model, trainloader, optimizer, scheduler, epoch, np.unique(train_set.targets), args
                    )
                    test_loss, test_acc, logs = test(self.model, testloader, args, session)
                ...
                if not args.not_data_init:
                    ...
                    self.trlog['max_acc'][session] = float('%.3f' % (test_acc * 100))
                    result_list.append(
                        'After Prototype FC: test_loss:%.5f,test_acc:%.5f\n' % (test_loss, test_acc)
                    )
            else:
                print("Incremental session: [%d]" % session)
                print("#" * 50)
                ...
                self.model.train()
                trainloader.dataset.transform = testloader.dataset.transform
                test_acc, novel_last_acc = self.model.train_inc(
                    trainloader, self.args.epochs_new, session, np.unique(train_set.targets), testloader, result_list, test, self.model
                )
